...
public class KozaShortStatistics extends SimpleShortStatistics
    ...
    protected long totalDepthSoFarTree[][];
    protected long totalSizeSoFarTree[][];
    protected long[][] totalSizeThisGenTree;                  // per-subpop total size of individuals this generation per tree
    protected long[][] totalDepthThisGenTree;                 // per-subpop total size of individuals this generation per tree
    //============
    ...
    protected void postInitializationStatistics_extensive(final EvolutionState state){
    	super.postInitializationStatistics(state);
    }
    ...
    protected void prepareStatistics(EvolutionState state)
        ...
        for(int x = 0 ; x < state.population.subpops.length; x++)
            {
            GPIndividual i = (GPIndividual)(state.population.subpops[x].individuals[0]);
            totalDepthThisGenTree[x] = new long[i.getTreesLength()]; //======zhixing, 2021.3.26
