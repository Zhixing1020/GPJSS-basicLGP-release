...
public class HalfBuilder extends KozaBuilder
    ...
    public GPNode newRootedTree(final EvolutionState state,
        ...
        {
    	//============zhixing, LGP in JSS, 2021.3.26
    	if(set.registers[type.type] != null && set.registers[type.type].length > 0){
    		if (state.random[thread].nextDouble() < pickGrowProbability)
                return growNode_reg(state,0,state.random[thread].nextInt(maxDepth-minDepth+1) + minDepth,type,thread,parent,argposition,set);
            else
                return fullNode_reg(state,0,state.random[thread].nextInt(maxDepth-minDepth+1) + minDepth,type,thread,parent,argposition,set);
    	}
    	//===============
    	if (state.random[thread].nextDouble() < pickGrowProbability)
    ...
    //==============Zhixing, Multiform LGP in JSS, 2022.9.26
    public GPNode newRootedSubTree(final EvolutionState state,
    		final int current,
            final GPType type,
            final int thread,
            final GPNodeParent parent,
    	...
    	if(set.registers[type.type] != null && set.registers[type.type].length > 0){
    		if (state.random[thread].nextDouble() < pickGrowProbability)
                return growNode_reg(state,current,state.random[thread].nextInt(maxDepth-Math.max(minDepth, current)+1) + Math.max(minDepth, current),type,thread,parent,argposition,set);
            else
                return fullNode_reg(state,current,state.random[thread].nextInt(maxDepth-Math.max(minDepth, current)+1) + Math.max(minDepth, current),type,thread,parent,argposition,set);
    	}
    	if (state.random[thread].nextDouble() < pickGrowProbability)
            return growNode(state,current,state.random[thread].nextInt(maxDepth-Math.max(minDepth, current)+1) + Math.max(minDepth, current),type,thread,parent,argposition,set);
        else
            return fullNode(state,current,state.random[thread].nextInt(maxDepth-Math.max(minDepth, current)+1) + Math.max(minDepth, current),type,thread,parent,argposition,set);
        }
    //=========================
