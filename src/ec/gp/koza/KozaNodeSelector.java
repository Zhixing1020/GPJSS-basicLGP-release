...
public class KozaNodeSelector implements GPNodeSelector 
    ...
    //===============zhixing, multiform LGP for DJSS, 2022.9.26
    public GPNode pickNode(final EvolutionState s,
            final int subpopulation,
            final int thread,
            final GPIndividual ind,
            final GPTree tree,
            final int GPNodeType) {
    	if (nodes==-1) nodes=tree.child.numNodes(GPNodeType);
    	
    	if(nodes > 0) {
    		return tree.child.nodeInPosition(s.random[thread].nextInt(nodes), GPNodeType);
    	}
