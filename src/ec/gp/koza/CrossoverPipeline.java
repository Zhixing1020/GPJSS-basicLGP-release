...
public class CrossoverPipeline extends GPBreedingPipeline
    ...
    public int produce(final int min, 
        ...
        for(int q=start;q<n+start; /* no increment */)  // keep on going until we're filled up
            ...
            if (tree1!=TREE_UNFIXED && (tree1<0 || tree1 >= parents[0].getTreesLength()))//========zhixing, 2021.3.26
                // uh oh
            ...
            if (tree2!=TREE_UNFIXED && (tree2<0 || tree2 >= parents[1].getTreesLength()))//========zhixing, 2021.3.26
                // uh oh
            ...
            if (tree1==TREE_UNFIXED || tree2==TREE_UNFIXED) 
                ...
                do
                    ...
                    if (tree1==TREE_UNFIXED) 
                        if (parents[0].getTreesLength() > 1) //========zhixing, 2021.3.26
                            t1 = state.random[thread].nextInt(parents[0].getTreesLength());//========zhixing, 2021.3.26
                        else t1 = 0;
                    else t1 = tree1;

                    if (tree2==TREE_UNFIXED) 
                        if (parents[1].getTreesLength()>1)//========zhixing, 2021.3.26
                            t2 = state.random[thread].nextInt(parents[1].getTreesLength());//========zhixing, 2021.3.26
                        else t2 = 0;
                    else t2 = tree2;
                    } while (parents[0].getTree(t1).constraints(initializer) != parents[1].getTree(t2).constraints(initializer));//========zhixing, 2021.3.26
                }
            else
                {
                t1 = tree1;
                ...
                if (parents[0].getTree(t1).constraints(initializer) //========zhixing, 2021.3.26
                    != parents[1].getTree(t2).constraints(initializer)) // uh oh   //========zhixing, 2021.3.26
            ...
            for(int x=0;x<numTries;x++)
                ...
                p1 = nodeselect1.pickNode(state,subpopulation,thread,parents[0],parents[0].getTree(t1));//========zhixing, 2021.3.26
                
                // pick a node in individual 2
                p2 = nodeselect2.pickNode(state,subpopulation,thread,parents[1],parents[1].getTree(t2)); //========zhixing, 2021.3.26
            ...
            if (n-(q-start)>=2 && !tossSecondParent) 
                for(int x=0;x<j2.getTreesLength();x++)
                    ...
                    if (x==t2 && res2)  // we've got a tree with a kicking cross position!
                        { 
                        tree = (GPTree)(parents[1].getTree(x).lightClone());           
                        tree.owner = j2;
                        ...
                        tree = (GPTree)(parents[1].getTree(x).lightClone());           
                        tree.owner = j2;
                        tree.child = (GPNode)(parents[1].getTree(x).child.clone());
                        tree.child.parent = tree;
                        tree.child.argposition = 0;
                        j2.setTree(x, tree);
                        }
