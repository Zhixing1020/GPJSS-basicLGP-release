...
public abstract class GPNode implements GPNodeParent, Prototype
    ...
    public static final int NODESEARCH_CONSTANT = 5;
    public static final int NODESEARCH_READREG = 4;
    public static final int NODESEARCH_NULL = 6;
    //================
    ...
    */
    ...
    public int numNodes(final int nodesearch)
        ...
        int s=0;
        for(int x=0;x<children.length;x++) {
        	if(children[x] != null) //=============zhixing, LGP for JSS, 2021.4.10
            	s += children[x].numNodes(nodesearch);
        	else if (nodesearch == NODESEARCH_NULL && children[x] == null) {
        		s += 1;
        	}
        }
        return s + ((nodesearch==NODESEARCH_ALL ||
                (nodesearch==NODESEARCH_TERMINALS && children.length==0) ||
                (nodesearch==NODESEARCH_NONTERMINALS && children.length>0 && !(this instanceof WriteRegisterGPNode))||
                //===============zhixing, LGP for JSS, 2021.4.10
                (nodesearch == NODESEARCH_CONSTANT && children.length == 0 && ! (this instanceof ReadRegisterGPNode))||
                (nodesearch == NODESEARCH_READREG && (this instanceof ReadRegisterGPNode)))
        		? 1 : 0);
        		//=================
        }
...
//            }
    ...
    */
    ...
    //=====================zhixing, LGP for JSS, 2021.4.7
    public void collectReadRegister(Set<Integer> s){
    	//if((this instanceof TerminalERC || this instanceof ReadRegisterGPNode) && ((TerminalERC)this).getTerminal() instanceof ReadRegisterGPNode)
    	if(this instanceof ReadRegisterGPNode)
    	{
    		//s.add(((ReadRegisterGPNode)((TerminalERC)this).getTerminal()).getIndex());
    		s.add(((ReadRegisterGPNode)this).getIndex());
    	}
    	for(int x=0;x<children.length;x++)
        {
			children[x].collectReadRegister(s);
        }
    }
    public void collectReadRegister_list(List<Integer> s){
    	//if((this instanceof TerminalERC || this instanceof ReadRegisterGPNode) && ((TerminalERC)this).getTerminal() instanceof ReadRegisterGPNode)
    	if(this instanceof ReadRegisterGPNode)
    	{
    		//s.add(((ReadRegisterGPNode)((TerminalERC)this).getTerminal()).getIndex());
    	...
    	for(int x=0;x<children.length;x++)
        {
			children[x].collectReadRegister_list(s);
        }
    }
    //============================
