...
public class RehangPipeline extends GPBreedingPipeline
    ...
    public int produce(final int min, 
        ...
        for(int q=start; q < n+start; q++)
            ...
            if (tree!=TREE_UNFIXED && (tree<0 || tree >= i.getTreesLength()))//========zhixing, 2021.3.26
                // uh oh
            ...
            if (sources[0] instanceof BreedingPipeline)
                ...
                for(int x=0;x<j.getTreesLength();x++)
                    ...
                    t = (GPTree)(i.getTree(x).lightClone());
                    t.owner = j;
            ...
            for (int x=0;x<numTries;x++)
                ...
                if (tree==TREE_UNFIXED)
                    if (i.getTreesLength()>1) t = state.random[thread].nextInt(i.getTreesLength());//========zhixing, 2021.3.26
                    else t = 0;
                else t = tree;
                
                // is the tree rehangable?              
                if (j.getTree(t).child.children.length==0) continue; // uh oh, try again  //========zhixing, 2021.3.26
                boolean rehangable = false;
                for(int y=0;y<j.getTree(t).child.children.length;y++)//========zhixing, 2021.3.26
                    if (j.getTree(t).child.children[y].children.length>0) // nonterminal   //========zhixing, 2021.3.26
                        { rehangable = true; break; }
                if (!rehangable) continue;  // the root's children are all terminal

                int numrehang = numRehangableNodes(j.getTree(t).child,0);//========zhixing, 2021.3.26
                pickRehangableNode(j.getTree(t).child,  
                    state.random[thread].nextInt(numrehang));//========zhixing, 2021.3.26
                
                rehang(state,thread,rehangableNode,j.getTree(t).child);//========zhixing, 2021.3.26
