...
public class InternalCrossoverPipeline extends GPBreedingPipeline
    ...
    public int produce(final int min, 
        ...
        for(int q=start;q<n+start; q++)
            ...
            if (tree1!=TREE_UNFIXED && (tree1<0 || tree1 >= i.getTreesLength())) //========zhixing, 2021.3.26
                // uh oh
            ...
            if (tree2!=TREE_UNFIXED && (tree2<0 || tree2 >= i.getTreesLength())) //========zhixing, 2021.3.26
                // uh oh
                state.output.fatal("Internal Crossover Pipeline attempted to fix tree.0 to a value which was out of bounds of the array of the individual's trees.  Check the pipeline's fixed tree values -- they may be negative or greater than the number of trees in an individual"); 
            ...
            if (sources[0] instanceof BreedingPipeline)
                ...
                for(int x=0;x<j.getTreesLength();x++)
                    {
                	GPTree t = j.getTree(x);
                    t = (GPTree)(i.getTree(x).lightClone());  // light clone
                    t.owner = j;
                    t.child = (GPNode)(i.getTree(x).child.clone());
                    t.child.parent = j.getTree(x);
            ...
            if (tree1==TREE_UNFIXED || tree2==TREE_UNFIXED) 
                ...
                do
                    ...
                    if (tree1==TREE_UNFIXED)
                        if (i.getTreesLength() > 1) //========zhixing, 2021.3.26
                            t1 = state.random[thread].nextInt(i.getTreesLength()); //========zhixing, 2021.3.26
                        else t1 = 0;
                    else t1 = tree1;
                    
                    if (tree2==TREE_UNFIXED) 
                        if (i.getTreesLength() > 1)//========zhixing, 2021.3.26
                            t2 = state.random[thread].nextInt(i.getTreesLength());//========zhixing, 2021.3.26
                        else t2 = 0;
                    else t2 = tree2;
                    } while (i.getTree(t1).constraints(initializer) != i.getTree(t2).constraints(initializer));//========zhixing, 2021.3.26
                }
            else
                {
                t1 = tree1;
                ...
                if (i.getTree(t1).constraints(initializer) //========zhixing, 2021.3.26
                    != i.getTree(t2).constraints(initializer)) // uh oh  //========zhixing, 2021.3.26
            ...
            for(int x=0;x<numTries;x++)
                ...
                p1 = nodeselect0.pickNode(state,subpopulation,thread,j,j.getTree(t1));//========zhixing, 2021.3.26
                
                // pick a node in individual 2
                p2 = nodeselect1.pickNode(state,subpopulation,thread,j,j.getTree(t2));//========zhixing, 2021.3.26
