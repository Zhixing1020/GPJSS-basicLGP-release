    ...
    public int produce(final int min, 
        ...
        for(int q=start; q < n+start; q++)
            ...
            for (int x=0;x<numTries;x++)
                ...
                int numpromote = numPromotableNodes(initializer, j.getTree(t).child,0);//========zhixing, 2021.3.26
                if (numpromote==0) continue; // uh oh, try again
                
                // promote the node, or if we're unsuccessful, just leave it alone
                pickPromotableNode(initializer, j.getTree(t).child,state.random[thread]. //========zhixing, 2021.3.26
                    nextInt(numpromote));
