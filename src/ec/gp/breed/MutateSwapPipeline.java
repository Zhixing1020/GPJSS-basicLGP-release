    ...
    public int produce(final int min, 
        ...
        for(int q=start; q < n+start; q++)
            ...
            for (int x=0;x<numTries;x++)
                ...
                GPInitializer initializer = ((GPInitializer)state.initializer);
                int numswap = numSwappableNodes(initializer, j.getTree(t).child,0); //========zhixing, 2021.3.26
                if (numswap==0) continue; // uh oh, try again
                
                // swap the node, or if we're unsuccessful, just leave it alone
                pickSwappableNode(initializer, j.getTree(t).child,state.random[thread].nextInt(numswap));//========zhixing, 2021.3.26
