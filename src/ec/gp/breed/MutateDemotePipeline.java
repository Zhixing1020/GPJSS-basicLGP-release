    ...
    public int produce(final int min, 
        ...
        for(int q=start; q < n+start; q++)
            ...
            for (int x=0;x<numTries;x++)
                ...
                int numdemote = numDemotableNodes(initializer, j.getTree(t).child,0,j.getTree(t).constraints(initializer).functionset);//========zhixing, 2021.3.26
                if (numdemote==0) continue; // uh oh, try again
                
                // demote the node, or if we're unsuccessful, just leave it alone
                pickDemotableNode(initializer, j.getTree(t).child,state.random[thread].nextInt(numdemote),j.getTree(t).constraints(initializer).functionset);//========zhixing, 2021.3.26
                
                // does this node exceed the maximum depth limits?
                if (!verifyPoint(demotableNode)) continue; // uh oh, try again
                
                // demote it
                demoteSomething(demotableNode,state,thread,j.getTree(t).constraints(initializer).functionset);//========zhixing, 2021.3.26
                j.evaluated = false;
                break;
                }
