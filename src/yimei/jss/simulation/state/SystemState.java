...
public class SystemState {
    ...
    //================2022.1.4   zhixing,  normalization for DJSS attributes
    private List<Double> maxValue;
    
    private void resetMaxValue(){
    	maxValue = new ArrayList<>();
    	
    	for(int i = 0; i<JobShopAttribute.values().length;i++){
    		maxValue.add(1.0);
    	}
    }
    
    public void determineMaxValue(){
    	double maxv = 0;
    	double tmp = 0;
    	for(JobShopAttribute a : JobShopAttribute.values()){
    		maxv = 0;
    		for(WorkCenter w : workCenters){
    			for(Operation op : w.getQueue()){
    				tmp = a.value(op, w, this);
    		...
    		maxValue.set(a.ordinal(), maxv);
    	}
    }
    
    public Double getMaxValue(int index){
    	return maxValue.get(index);
    }
    //=================
    ...
    public void reset() {
        ...
        resetMaxValue(); //=================2022.1.4   zhixing,  normalization
    }
